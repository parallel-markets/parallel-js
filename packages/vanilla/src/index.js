...
const addScript = () => {
    ...
    if (!headOrBody)
        throw new Error('Parallel JS cannot be added to page with no <head/> or <body/> element');
    headOrBody.appendChild(script);
    return script;
};
const loadScript = () => {
    // Load parallel script at most once
    if (parallelPromise !== undefined)
...
export const loadParallel = (config) => {
    ...
    return loadScript().then((Parallel) => {
        ...
        return new Promise((resolve) => {
            ...
            Parallel.init(Object.assign(Object.assign({}, config), { on_init: onInit }));
        });
    });
};
